...
// 生成启动命令
function buildCommand(config: any): string {
  const parts: string[] = [];
...
ipcMain.handle('check-environment', async () => {
  // 检测 Node.js
  const nodeStatus = await new Promise<{ installed: boolean; version?: string }>((resolve) => {
  ...
  const npxStatus = await new Promise<boolean>((resolve) => {
  ...
  // 检测 agent-tars
  const agentTarsStatus = await new Promise<{ installed: boolean; version?: string }>((resolve) => {
